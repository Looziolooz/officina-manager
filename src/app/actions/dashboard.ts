...
export async function getDashboardData() {
  ...
  const [kpis, chartData, topParts, topCustomers, quickStats, revenueSplit] = await Promise.all([
    getDashboardKPIs(),
...
// 1. KPI Principali
// Nota: i delta sono calcolati SOLO quando abbiamo il dato storico reale.
// Fatturato e margine si confrontano con ieri (dati disponibili).
// "Auto in officina" e "Valore magazzino" NON hanno uno storico salvato,
// quindi non mostrano una variazione inventata (vedi TODOS.md T3 per gli snapshot).
async function getDashboardKPIs() {
  ...
  const [todayData, yesterdayData, warehouseData, carsData] = await Promise.all([
  ...
  const yesterdayMargin = yesterdayRevenue - (yesterdayData._sum.amount || 0);
  const marginChange = yesterdayMargin > 0 ? ((todayMargin - yesterdayMargin) / yesterdayMargin) * 100 : 0;
...
// 1-bis. Distribuzione entrate REALE (ultimi 30 giorni): manodopera vs ricambi.
// Manodopera = somma Job.laborCost dei lavori completati; ricambi = somma
// (appliedPrice x quantity) dei pezzi montati. Niente split inventato.
async function getRevenueSplit() {
  const last30Days = subDays(new Date(), 30);

  const [laborData, partsRows] = await Promise.all([
    prisma.job.aggregate({
      where: { completedAt: { gte: last30Days } },
      _sum: { laborCost: true },
    }),
    prisma.partOnJob.findMany({
      where: { createdAt: { gte: last30Days } },
      select: { appliedPrice: true, quantity: true },
    }),
  ]);

  const labor = laborData._sum.laborCost || 0;
  const parts = partsRows.reduce((sum, r) => sum + r.appliedPrice * (r.quantity ?? 0), 0);
