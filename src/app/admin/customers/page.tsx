  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Link href="/admin/customers/new" className="bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center gap-2">
          <Plus size={20} /> Nuovo
        </Link>
      </div>
            ...
            {customers.map((customer) => (
              <tr key={customer.id} className="hover:bg-white/2 transition-colors">
                ...
                <td className="p-4 text-right">
                  <Link href={`/admin/customers/${customer.id}`} className="text-blue-400"><ArrowRight size={20} /></Link>
                </td>
              </tr>
            ))}
          </tbody>
