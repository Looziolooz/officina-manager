...
export default async function CustomerDetailPage({
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-8">
      ...
      {/* A) Header Card */}
      <div className="bg-linear-to-r from-slate-900 to-slate-950 border border-border p-8 rounded-3xl relative overflow-hidden">
        <div className="absolute top-0 right-0 p-32 bg-primary/5 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 pointer-events-none" />
        
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative z-10">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <h1 className="text-4xl font-bold text-foreground tracking-tight">{customer.firstName} {customer.lastName}</h1>
            ...
            <div className="flex gap-6 text-muted-foreground">
              ...
              {customer.email && (
                <a href={`mailto:${customer.email}`} className="flex items-center gap-2 hover:text-foreground transition-colors">
          ...
          <div className="flex gap-3">
             <Link 
                ...
                className="bg-background hover:bg-background border border-border text-foreground px-6 py-3 rounded-xl font-medium transition-all flex items-center justify-center"
             >
                Modifica Profilo
             </Link>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-background border border-border p-6 rounded-2xl">
          ...
          <p className="text-3xl font-mono font-bold text-success">€ {(customer.totalSpent ?? 0).toFixed(2)}</p>
        </div>
        <div className="bg-background border border-border p-6 rounded-2xl">
          <div className="flex items-center gap-3 mb-2 text-muted-foreground">
          ...
          <p className="text-3xl font-mono font-bold text-blue-600">€ {(customer.totalMargin ?? 0).toFixed(2)}</p>
        </div>
        <div className="bg-background border border-border p-6 rounded-2xl">
          <div className="flex items-center gap-3 mb-2 text-muted-foreground">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {customer.vehicles.map(vehicle => (
                <div key={vehicle.id} className="bg-surface border border-border p-5 rounded-2xl group hover:border-primary/50 transition-colors">
                  ...
                  <p className="text-foreground font-bold text-lg">{vehicle.brand} {vehicle.modelName}</p>
                  <p className="text-sm text-muted-foreground mb-4">VIN: {vehicle.vin || 'N/D'} • {vehicle.fuelType || 'N/D'}</p>
        ...
        <div className="bg-background border border-border p-6 rounded-3xl h-fit">
          <h3 className="text-lg font-bold text-foreground mb-6 uppercase tracking-widest">Ultimi Lavori</h3>
          <div className="space-y-6 relative border-l border-border ml-3 pl-6">
            {customer.vehicles.flatMap(v => v.jobs).map(job => (
              <div key={job.id} className="relative">
                ...
                <p className="text-xs text-muted-foreground mb-1">
                  {job.createdAt?.toLocaleDateString('it-IT') || 'N/D'}
                </p>
                <h4 className="text-foreground font-bold text-sm">{job.title}</h4>
                <div className="flex justify-between items-center mt-2">
                  <span className="text-[10px] bg-green-500/20 text-success px-2 py-0.5 rounded border border-green-500/20">
                    {job.status}
                  </span>
