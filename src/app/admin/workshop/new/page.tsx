  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 text-foreground">
      ...
      <div className="relative">
        ...
        <input type="text" value={query} onChange={searchVehicle} placeholder="Targa o Cognome..." className="w-full bg-background border border-border rounded-xl p-4 pl-12" />
      </div>
      <div className="space-y-4">
        {results.map((vehicle) => (
          <div key={vehicle.id} className="bg-background border border-border p-5 rounded-xl flex items-center justify-between">
            <div className="flex items-center gap-5">
              <Car size={24} className="text-blue-500" />
              <div>
                <p className="text-2xl font-black uppercase">{vehicle.plate}</p>
                <p className="text-muted-foreground">{vehicle.brand} {vehicle.modelName}</p>
              </div>
              <div className="h-10 w-px bg-background mx-4" />
